...
// Menyajikan semua file statis seperti index.html, script.js, dll. dari folder utama
...
app.get('/api/cuaca', async (req, res) => {
  try {
    ...
    parseString(response.data, (err, result) => {
      if (err) { return res.status(500).json({ message: 'Gagal memproses data XML dari BMKG.' }); }
      const areaBekasi = result.data.forecast[0].area.find(a => a.$.description === 'Bekasi');
      if (!areaBekasi) { return res.status(404).json({ message: 'Data cuaca Bekasi tidak ditemukan.' }); }
      ...
      const cuacaData = { kota: 'Bekasi', temperatur: `${temperatur}°C`, cuaca: getNamaCuaca(cuacaCode) };
      res.json(cuacaData);
    });
  } catch (error) {
...
function getNamaCuaca(code) {
    const kodeCuaca = { '0': 'Cerah', '1': 'Cerah Berawan', '3': 'Berawan', '4': 'Berawan Tebal', '60': 'Hujan Ringan', '61': 'Hujan Sedang', '63': 'Hujan Lebat' };
